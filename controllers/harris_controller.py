...
class HarrisController(QObject):
    ...
    def _set_spinbox_defaults(self) -> None:
        ...
        defs = {
            self._SPN_THRESHOLD:  (0.01,  0.001,  1.0,  3, 0.01),  # added step
            self._SPN_K:          (0.04,  0.01,   0.2,  3, 0.01),   # added step  
            self._SPN_MAXCORNERS: (2000,  100,    10000, 0, 100),    # added step
        }
        for name, (val, mn, mx, dec, step) in defs.items():  # unpack step
            w = self._widget(name)
            if w is None:
                continue
            w.setMinimum(mn)
            w.setMaximum(mx)
            w.setSingleStep(step)  # set custom step size
            if dec > 0:
                w.setDecimals(dec)
            w.setValue(val)
