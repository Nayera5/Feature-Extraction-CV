...
# ------------------------------------------------------------------
# Canonical size: both images are resized to this BEFORE SIFT runs.
# Matching and visualisation use coordinates in this space.
# ------------------------------------------------------------------
CANONICAL_W = 400
CANONICAL_H = 300
...
class MatchingWorker(QThread):
    ...
    Both images are resized to (CANONICAL_W, CANONICAL_H) before feature
    extraction so that keypoint coordinates are directly comparable and
    map correctly onto the visualisation canvas.
    """
    ...
    def run(self):
        try:
            ...
            # ── Step 0: resize BOTH images to the same canonical size ──────
            img1_canon = cv2.resize(self.image1, (CANONICAL_W, CANONICAL_H),
                                    interpolation=cv2.INTER_AREA)
            img2_canon = cv2.resize(self.image2, (CANONICAL_W, CANONICAL_H),
                                    interpolation=cv2.INTER_AREA)

            # ── Step 1: extract SIFT features on the canonical images ──────
            t_extract_start = time.perf_counter()
            result1 = self.sift_controller.run(img1_canon)          # already BGR
            result2 = self.sift_controller.run(img2_canon)
            ...
            self.finished.emit({
                "image1_canon":   img1_canon,
                "image2_canon":   img2_canon,
...
class MatchingImagesController(QObject):
    ...
    def __init__(self, parent: QMainWindow = None):
        ...
        self.image1_canon:       np.ndarray | None = None   # canonical-size BGR
        self.image2_canon:       np.ndarray | None = None
    ...
    def _display_fit_in_label(self, label, image_rgb: np.ndarray):
        """Fit image into label without any scrolling — scales to label size."""
        try:
    ...
    def _on_matching_finished(self, result: dict):
        try:
            # Cache canonical images for visualisation
            self.image1_canon       = result["image1_canon"]
            self.image2_canon       = result["image2_canon"]
            ...
            self.window.matchingStatsLbl.setText(
                ...
                f"SIFT Time:         {result['extraction_sec']:.3f} s\n"
                f"Match Time:        {result['matching_sec']:.3f} s\n"
                f"Total Time:        {result['total_sec']:.3f} s"
            )
            self._draw_matches()
            self.status_message.emit(
                f"Done: {result['num_matches']} matches in {result['total_sec']:.3f}s "
                f"(SIFT {result['extraction_sec']:.3f}s)."
            )
    ...
    def _draw_matches(self):
        """
        Render side-by-side canonical images with keypoint circles and match lines.

        Both images are already (CANONICAL_W × CANONICAL_H) so keypoint
        coordinates map directly — no scaling arithmetic needed.
        The combined canvas is then scaled to fit the host label exactly.
        """
        ...
        try:
            # ── Build side-by-side canvas from canonical BGR images ─────────
            img1 = self.image1_canon.copy()   # (CANONICAL_H × CANONICAL_W, BGR)
            img2 = self.image2_canon.copy()
            combined = np.hstack([img1, img2])   # (CANONICAL_H × 2*CANONICAL_W)

            num_to_show = min(
                self.window.matchingNumDisplaySpin.value(),
            ...
            for idx_a, idx_b in self.current_matches[:num_to_show]:
                ...
                # Shift pt2 right by one image width
                pt2_shifted = (pt2[0] + CANONICAL_W, pt2[1])
            ...
            # ── Fit into host label (no scrollbar needed) ───────────────────
            self._display_fit_in_label(
                self.window.matchingVisualizationHost, combined_rgb
            )
